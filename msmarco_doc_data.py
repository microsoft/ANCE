...
from ann_utils import multi_file_process, numbered_byte_file_generator, EmbeddingCache
import pickle
import gzip
...
def write_query_rel(args, pid2offset, query_file, positive_id_file, out_query_file, out_id_file):
    ...
    with open(out_query_path, 'wb') as f:
        for record in numbered_byte_file_generator(out_query_path, 32, 8 + 4 + args.max_query_length * 4):
            ...
            if not q_id in query_positive_id:
...
def preprocess(args):
    ...
    if args.data_type == 0: 
        write_query_rel(args, pid2offset, "msmarco-doctrain-queries.tsv.gz", "msmarco-doctrain-qrels.tsv.gz", "train-query", "train-qrel.tsv")
        write_query_rel(args, pid2offset, "msmarco-test2019-queries.tsv.gz", "2019qrels-docs.txt", "dev-query", "dev-qrel.tsv")
    else:
        write_query_rel(args, pid2offset, "queries.train.shuf.tsv", "qrels.train.tsv", "train-query", "train-qrel.tsv")
...
def PassagePreprocessingFn(args, line, tokenizer):
    if args.data_type == 0:
        ...
        full_text = p_text[:args.max_doc_character] # keep only first 10000 characters, should be sufficient for any experiment that uses less than 500 - 1k tokens
